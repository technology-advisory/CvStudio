...
function mailPayload(requireEmail=true){
  ...
  const body={to:$('#recipient-email').value.trim(),recipientName:$('#recipient-name').value.trim(),subject:$('#mail-subject').value.trim(),message:$('#mail-message').value.trim(),singleUse:$('#single-use').checked,maxDownloads:Number($('#max-downloads').value)};
...
function showGenerated(data){
  $('#generated-url').value=data.url;$('#open-generated').href=data.url;$('#generated-expiry').textContent=`Válido hasta ${formatDate(data.expiresAt)} · ${data.maxDownloads} descarga(s)`;$('#generated').classList.remove('hidden');
}
...
function renderStatistics(){
  ...
  $('#statistics-sends-body').innerHTML=sends.length?sends.map(x=>`<tr>
    ...
    <td>${x.download_count}/${x.max_downloads}</td>
  ...
  $('#statistics-downloads-body').innerHTML=downloads.length?downloads.map(x=>`<tr>
    ...
    <td><strong>${escapeHtml(x.ip_address||'No disponible')}</strong><br><small>${escapeHtml([x.city,x.country].filter(Boolean).join(', ')||'Sin geolocalización')}</small></td>
  ...
  </tr>`).join(''):'<tr><td colspan="5" class="empty">Todavía no se ha descargado ningún documento.</td></tr>';
}
...
function renderStats(){['ACTIVE','USED','EXPIRED','REVOKED'].forEach(s=>$('#stat-'+s.toLowerCase()).textContent=state.links.filter(x=>x.status===s).length);}
...
function renderLinks(){
  ...
  $('#links-body').innerHTML=rows.length?rows.map(x=>`<tr>
    ...
    <td>${formatDate(x.created_at)}</td><td>${formatDate(x.expires_at)}</td><td>${x.download_count}/${x.max_downloads}</td>
    <td><span class="pill ${x.status}">${label(x.status)}</span></td>
